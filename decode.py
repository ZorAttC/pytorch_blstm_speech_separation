...
def decode():
    ...
    for inputs,labels1,labels2 in train_loader:
        ...
        savepath1 = os.path.join("output", f"signal{i}" + '_1.wav')
        savepath2 = os.path.join("output", f"signal{i}" + '_2.wav')
...
if __name__ =="__main__":
    ...
    config=FLAGS
    param_num=30
    inputs=np.load("data\\tfrecords\\tr\\feats.npy",allow_pickle=True)[:param_num]
    inputs_phase=np.load("data\\tfrecords\\tr\\feats_phase.npy",allow_pickle=True)[:param_num]
